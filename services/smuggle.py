...
def shipment_seconds(qty: int) -> int:
    """زمان ارسال محموله به ثانیه، هر واحد محصول یه کم طولانی‌ترش می‌کنه"""
    return config.SHIPMENT_BASE_SECONDS + config.SHIPMENT_PER_UNIT_SECONDS * qty
...
def shipment_confirm_text(crop: str, qty: int, value: int) -> str:
    """کارت تایید ارسال محموله، قالب درخواستی کارفرما"""
        ...
        if sh.outcome == "delayed" and not sh.hops:
            ...
            out.append({
                ...
                "text": (
                    ...
                    f"با تأخیر میرسه، {fa_dur(shipment_seconds(sh.qty))} دیگه"
                ),
            })
            continue
